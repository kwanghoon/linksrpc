...
family for the purpose of integrating polymorphism with {\it
  effectful} features, such as mutable references, without breaking
type safety.

%
In the polymorphic RPC calculus, value restriction also provides a
simple mean to integrate polymorphism with a somewhat unusal effectful
feature of the statically located context of application terms.

\subsection{Why is naively relaxing value restriction on polymorphism dangerous?}

%
Suppose the following piece of a term is located in the client.
\[
\lambda^c z. \ \cdots \ \mbox{let} \  g \ = \ \Lambda\alpha. \ ( \  (\lambda^\server x:Int. \ \lambda^\server y:\alpha. \ y) \ 123  \ ) \ \mbox{in} \ ...
\]
%
The type of a polymorphic term $g$ is
$\forall\alpha.\alpha\funL{\server}\alpha$.
%
...
%
This explains why realaxing value restriction in the polymorphic RPC
calculus can be dangerous to statically located contexts of
applications.
...
conflict arises.
%
This enforcment can be implemented by having a fresh location $l_0$
for typing the body of type and location abstractions as follows.
%
\[
    \begin{prooftree}
      \hypo{  \typing{\tyenv}{l_0}{M}{A} \ \ \ \mbox{fresh} \ l_0}
      \infer[left label=(T-Tabs)]1{ \typing{\tyenv}{\Loc}{\Lambda\alpha. M}{\forall\alpha. A}   }
    \end{prooftree}
    \ \ \
    \begin{prooftree}
      \hypo{ \typing{\tyenvExtWith{l}}{l_0}{M}{A}  \ \ \ \mbox{fresh} \ l_0}
      \infer[left label=(T-Labs)]1{ \typing{\tyenv}{\Loc}{\Lambda l. M}{\forall l. A }}
    \end{prooftree}    
\]
%    
Recall that the existing typing rules have $\Loc$ instead of the fresh
location variable.
%
...
%
Users have only to adopt the two typing rules (T-Tabs) and (T-Labs) to
make use of relaxed polymorphic RPC calculuse.
%
But to prove whether this idea does make the relaxed calculus be type
safe involves more, which will be explained in the next section.
...
\end{tabular}
\caption{The polymorphic  RPC calculus with no value restriction on polymorphism: \polyrpcM}
\label{fig:polyrpcm}
\end{figure}
...
\end{tabular}
\caption{A type system for \polyrpcM}
