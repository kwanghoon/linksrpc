...
occurring in the map function types would be assumed to have the same
location annotation.
%
The following signature, though Links does not allow such a function
type, describes the idea of this heuristic.
%
For notation, $a \funL{client} b$ is a type of functions that are
guaranteed to run in the client.
...
%
The Links signaure should be interpreted similarly as the heuristic.
%
That is, it would be viewed as $(a \funL{l} b, [a]) \funL{l} [b]$.
%

In this example, the polymorphic map function can still run as a local
computation wherever it chooses to run by $map \ Client$ or $map
\ Server$.
%
There is no need to refer to $z$ in the map example.

However, sometimes there is a need to check locations in runtime.
%
For
this purpose, the polymorphic map function is written to take a value
for representing locations as an argument.
%
