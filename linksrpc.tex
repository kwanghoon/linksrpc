...
\section{Relaxing the Value Restriction of the Polymorphic RPC Calculus}
\label{app:2}

In the polymorphic RPC calculus, the body of location and type
abstractions is restricted to values as $\Lambda l. V$ and
$\Lambda\l.V$.
%
This is a form of the value restriction, which is well-known in the ML
programming language family, that, for example, prevents reference
cells from holding values of different types and preserves type
safety.
%
The value restriction in the polymorphic RPC calculus is not only
about type safe polymorphic references but, more importantly, it is
also about the statically located context of application terms.
%
...
calculus at all.
%
This section will discuss why the relaxation can break the type
soundness and how the value restriction can be relaxed still
preserving the type soundness.

\subsection{An Account for the Value Restriction}

% Application terms with locations : M N @ Loc (M must be applied to n only at Loc)
% Prove the type soundness again.

\subsection{Breaking the Type Soundness by Relaxation}
...
\subsection{An Effectful Polymorphic RPC Calculus with No Value Restriction}
