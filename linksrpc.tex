...
%
A heuristic could be adopted to reflect the programmer's view by the
presence of the client location annotation to all function types
occurring in the map function types.
%
The following signature, though Links does not allow to write such a
function type, describes the idea of this interpretation where
functions of type $a \funL{client} b$ is guaranteed to run in the
client.
...
some way to make this function run in the client map function without
runtime checking.
%
...
\]
%
in the Links syntax\footnote{Actually, Links does not allow to have
  location annotations in anonymous functions.}.
%
This function is of type $a\funL{client} b$ as the signature.
%
...
Note that the map function is a location-neutral one where it should
run both of the client and the server. So, in Links, the map function
is written without location annotation as follows.
...
%
The polymorphic map function in Links would be represented as follows.
...
%
Here, the polymorphic map function takes a value for representing
locations as an argument.
%
When runtime location checking is necessary, the argument $z$ could be
examined.
%
But in this example, the map function can still run as a local
computation wherever it chooses to run by $map \ Client$ or $map
\ Server$.
%
There is no need to refer to $z$ in the map example.
%
